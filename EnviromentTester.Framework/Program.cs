...
namespace EnviromentTester.Framework
{
     public class EnvironmentTester
    {
        ...
        public static async Task MainAsync(string[] args)
        {
            ...
            var d = await ExtractData();

            await PrintLineAsync(d.CertificateStatus.ToString());
        ...
        static async Task<CertificateAndStatus> ExtractData()
        {
            var logonHandler = ServiceProvider.GetRequiredService<ILogonHandler>();

            return await logonHandler.ValidateAndExtractCertificateAndStatus(loginData, challenge, _logOnto);
        }
        static Task<string> FindOcspUrlInCertificateAsync(X509Certificate2 cert)
        {
            var x509CertificatePropertyExtrator = ServiceProvider.GetRequiredService<IX509CertificatePropertyExtrator>();
